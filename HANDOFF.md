...
**Nothing is blocking. Next task is the customer section.**
...
- 2,400 ghost customer rows deleted. `shopify_customers` is back to 19,163 with
  zero rows lacking `customer_created_at`. `loyalty-join.mjs` is now UPDATE-only
  and verified: 2,401 LoyaltyLion customers with no Shopify counterpart are
...
decline runs from 2021 H2 with no discontinuity at February 2026. But capture
at the till collapsed — new POS customers per 100 POS orders went 12.5 (2022)
to 3.2 (2024).
...
1. **Customer section** — carry the "flat because mix gain offset decay"
   framing into the UI so the cohort chart cannot be misread, and surface
   enrolment as a retention dimension
...
5. **`/v2/orders`** — never used, carries cancellation status, refunds and
   channel. A third independent view of every order.
